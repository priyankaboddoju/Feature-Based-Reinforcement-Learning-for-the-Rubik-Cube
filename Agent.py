...
class Agent:
    ...
    def __init__(self, QValues=None, cube=None, replay_buffer=[]):
        self.visited = []
        self.visit_count = {}
        self.revisits = 0
    ...
    # explore
    def QLearn(self, discount=0.95, episodes=10, epsilon=0.9):
        ...
        for i in range(episodes):
            ...
            if follow_policy > epsilon:
                ...
                for action in self.actions:
                    if (
                        self.QV[(self.curr_state.__hash__(), action)] > best_QV
                ...
                for action in self.actions:
                    ...
                    self.QV[(self.curr_state.__hash__(), action)] = curr_QV + ALPHA * (
    ...
    def Play(self):
        ...
        for i in range(20):
            ...
            if not (self.curr_state.__hash__(), self.actions[0]) in self.QV.keys():
                ...
                for action in self.actions:
                    if self.QV[(self.curr_state.__hash__(), action)] > best_QV and (
            ...
            print(
                "q value is " + str(self.QV[(self.curr_state.__hash__(), best_action)])
            )
            # time.sleep(1)
            self.curr_state.move(best_action)
            self.second_last_action = self.last_action
    ...
    def get_success_rate(self, total_episodes):
        return self.successful_episodes / total_episodes * 100

    def experience_replay(self, replay_buffer, batch_size=32):
        ...
        if len(self.replay_buffer) >= batch_size:
            ...
            for exp in experiences:
                ...
                saved_rewards = state.__hash__() in self.R.keys()
                if not saved_rewards:
                    self.R[state.__hash__()] = []
                if not state.__hash__ in self.visit_count:
                    self.visit_count[state.__hash__()] = 1
                else:
                    self.visit_count[state.__hash__()] += 1
                vc = self.visit_count[state.__hash__()]

                for action in self.actions:
                    if not (state.__hash__(), action) in self.QV.keys():
                        self.QV[(state.__hash__(), action)] = 0
                    else:
                        self.revisits += 1
                        break
                    if not saved_rewards:
                        self.R[state.__hash__()].append(self.reward(state, action))
                if 100 in self.R[state.__hash__()]:
                    print("REACHED GOAL, END QLEARN ITERATION")
                    return
                ...
                if follow_policy > epsilon:
                    ...
                    for action in self.actions:
                        if (
                            self.QV[(state.__hash__(), action)] > best_QV
                            and action != self.last_action
                            and action != self.second_last_action
                        ):
                            best_action = action
                            best_QV = self.QV[(state.__hash__(), action)]
                    ...
                    for action in self.actions:
                        curr_QV = self.QV[(state.__hash__(), action)]
                        reward = self.reward(state, action)
                        max_reward = self.max_reward(state, action)
                        self.QV[(state.__hash__(), action)] = curr_QV + ALPHA * (
                            reward + (discount**vc) * max_reward - curr_QV
                        )
                    print(
                        "new q value for "
                        + best_action
                        + " action is "
                        + str(self.QV[(state.__hash__(), best_action)])
                    )

                    next_state.move(best_action)
                    next_state = next_state.copy()
                    if next_state.isGoalState():
                        print(
                            "reached goal state while in Q-learning episode " + str(i)
                        )
                        return
                    self.second_last_action = self.last_action
                    self.last_action = best_action
                    ...
                    self.move[action] = self.move[action] + 1
                    while (
                        action == self.last_action or action == self.second_last_action
                    ):
                        action = random.choice(self.actions)
    ...
    def train_ensemble(self, num_agents=5):
        ensemble_agents = [Agent() for _ in range(num_agents)]
...
for i in range(5):
    for j, e in enumerate(Epsilons):
        print("======= ROUND " + str(j) + "=========")
        agent.QLearn(epsilon=max(0.1, Epsilons[j] - 0.01))
...
agent.QLearn()
replay_buffer = []
agent.experience_replay(replay_buffer, batch_size=32)
total_episodes = 2 * len(Epsilons)  # Assuming two rounds of training for each epsilon
success_rate = agent.get_success_rate(total_episodes)
